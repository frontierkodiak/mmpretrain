_base_ = [
    '../_base_/datasets/reptiliaMulti_bs64_swin_224_augmented.py',
    '../_base_/schedules/imagenet_bs1024_adamw_swin.py',
    '../_base_/default_runtime.py'
]

train_dataloader = dict(batch_size=194)
val_dataloader = dict(batch_size=194)
...
model = dict(
    ...
    head=dict(  # Multi-task head
        type='MultiTaskHead',
        task_heads={
            'L10': dict(type='LinearClsHead', num_classes=237, in_channels=768, 
                        loss=dict(type='LabelSmoothLoss', label_smooth_val=0.1, mode='original'), 
                        init_cfg=[dict(type='TruncNormal', layer='Linear', std=2e-5)]),
            'L20': dict(type='LinearClsHead', num_classes=105, in_channels=768, 
                        loss=dict(type='LabelSmoothLoss', label_smooth_val=0.1, mode='original'), 
                        init_cfg=[dict(type='TruncNormal', layer='Linear', std=2e-5)]),
            'L30': dict(type='LinearClsHead', num_classes=35, in_channels=768, 
                        loss=dict(type='LabelSmoothLoss', label_smooth_val=0.1, mode='original'),
                        init_cfg=[dict(type='TruncNormal', layer='Linear', std=2e-5)]),
            'L40': dict(type='LinearClsHead', num_classes=3, in_channels=768, 
                        loss=dict(type='LabelSmoothLoss', label_smooth_val=0.1, mode='original'),
                        init_cfg=[dict(type='TruncNormal', layer='Linear', std=2e-5)]),
        },
    ),
    ...
    vis_backends=[
        ...
        dict(type='WandbVisBackend',
            init_kwargs={
                'project': 'Mr1A2',
                'group': 'aves_multi',
                'name': 'E21.1.vit-base-p16_8xb128-coslr-100e_reptiliaMulti'
            })
    ]
)
