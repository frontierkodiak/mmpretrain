_base_ = [
    '../_base_/datasets/avesMulti_bs64_swin_224_wsl.py',
...
model = dict(
    ...
    backbone=dict(
        ...
        final_norm=False,
        init_cfg=dict(type='Pretrained', checkpoint="~/local-data/modelZoo/mmpretrain/mae_vit-base-p16_8xb512-coslr-300e-fp16_in1k_20220829-c2cf66ba.pth")),
    neck=None,
    head=dict(  # Multi-task head
        type='MultiTaskHead',
        task_heads={
            'L10': dict(type='LinearClsHead', num_classes=871, in_channels=768, 
                        loss=dict(type='LabelSmoothLoss', label_smooth_val=0.1, mode='original'), 
                        init_cfg=[dict(type='TruncNormal', layer='Linear', std=2e-5)]),
            'L20': dict(type='LinearClsHead', num_classes=407, in_channels=768, 
                        loss=dict(type='LabelSmoothLoss', label_smooth_val=0.1, mode='original'), 
                        init_cfg=[dict(type='TruncNormal', layer='Linear', std=2e-5)]),
            'L30': dict(type='LinearClsHead', num_classes=94, in_channels=768, 
                        loss=dict(type='LabelSmoothLoss', label_smooth_val=0.1, mode='original'),
                        init_cfg=[dict(type='TruncNormal', layer='Linear', std=2e-5)]),
            'L40': dict(type='LinearClsHead', num_classes=25, in_channels=768, 
                        loss=dict(type='LabelSmoothLoss', label_smooth_val=0.1, mode='original'),
                        init_cfg=[dict(type='TruncNormal', layer='Linear', std=2e-5)]),
        },
    ),
    ...
    vis_backends=[
        ...
        dict(type='WandbVisBackend',
            init_kwargs={
                'project': 'Mb1A2',
                'group': 'aves_multi',
                'name': 'E21.1.wsl.vit-base-p16_8xb128-coslr-100e_avesMulti'
            })
    ]
)
